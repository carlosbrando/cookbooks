...
describe 'ruby::default' do
  ...
  let(:chef_run) { runner.converge 'ruby::default' }
  ...
  it "downloads rubygems" do
    chef_run.should create_remote_file "#{Chef::Config[:file_cache_path]}/rubygems-#{chef_run.node[:ruby][:rubygems][:version]}.tgz"
  end
